...
var DinnerModel = function() {
	...
	for (i in menu){
		if(id.Category == menu[i].Category){
			this.removeDishFromMenu(menu[i].Title);
		}
	...
	var url ="http://api.bigoven.com/recipes"+"?api_key="+apiKey+"&pg=2&rpp=50&include_primarycat="+type;
    if(filter){
        url += "&title_kw=" + filter;
        console.log(url);
        }
	...
	$.ajax({
        ...
        success: function (data) {
            ...
            notifyObservers(data.Results);
                ...
                $(".getAllDishes").html("Cant find dish: <span style='color:red; font-weight: bold;'>" + filter + "</span>.<br>"+" Search for other key word!");
            }
	...
	var url = "http://api.bigoven.com/recipe/"+id+"?api_key="+apiKey+"&pg=1&rpp=50";
